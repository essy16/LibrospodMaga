...
class MysteryBooksCases @Inject constructor(val bookRepository: BookRepository) {
     operator fun invoke(): Flow<Resource<List<Books>>> = flow{

        try {
            emit(Resource.Loading())
            val mysteryBooks = bookRepository.getMysteryBooks().map { it.toBooks() }
            emit(Resource.Success(mysteryBooks))
