...
class TravelBooksCases @Inject constructor(val bookRepository: BookRepository) {
    suspend operator fun invoke(): Flow<Resource<List<Books>>> = flow {
        try {
            emit(Resource.Loading())
            val travelBooks=bookRepository.getTravelBooks().map { it.toBooks() }
            emit(Resource.Success(travelBooks))
